// ============================================================
//  JNEET+ AI — pages/Dashboard.jsx  (Production v2.0)
// ============================================================
...
import {
  ...
  LogOut, ChevronRight, Sparkles, BookOpen,
...
function FeatureCard({ icon, title, desc, badge, badgeClass, available, onClick }) {
  return (
    <div
      ...
      className={`bg-bg-card border rounded-2xl p-5 flex items-center gap-4
        transition group
        ${available
    ...
    >
      <div className="w-11 h-11 rounded-xl bg-bg-panel flex items-center
        justify-center shrink-0 group-hover:scale-105 transition duration-150">
        {icon}
      </div>
...
export default function Dashboard() {
  const { user, examMode, logout } = useAuth();
  const navigate = useNavigate();

  const handleLogout = async () => {
    await logout();
    toast.success("Logged out! Phir milenge 👋");
    navigate("/login", { replace: true });
  };

  const handleModeSwitch = async (mode) => {
    if (mode === examMode) return;
    // Mode is stored in the user's DB record — switching requires backend call
    // For now: direct to ask page since mode is server-driven via JWT
  ...
  return (
    <div className="min-h-screen bg-bg-base text-white">
      ...
      {/* ── Navbar ──────────────────────────────────────── */}
      <nav className="border-b border-bg-border bg-bg-surface px-5 py-3.5
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-gray-500 hover:text-white
              border border-bg-border hover:border-bg-hover
      ...
      <div className="max-w-3xl mx-auto px-5 py-8 animate-fade-up">
        ...
        <div className="bg-bg-card border border-bg-border rounded-2xl p-5 mb-5">
          <p className="text-[10px] text-gray-700 mb-3 font-medium uppercase tracking-widest">
            Exam Mode (current)
          </p>
            ...
            {["NEET", "JEE"].map((mode) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl
                  font-semibold text-sm transition active:scale-[0.97]
                  ${examMode === mode
        ...
        {/* Exam countdown */}
        <div className="mb-5">
          <ExamCountdown examMode={examMode} />
        </div>
